...
from jacta.planner.learning.networks import Actor, Critic, train_actor_critic, train_actor_imitation, train_critic_imitation
...
from jacta.planner.planner.clipping_method import clip_actions
from jacta.planner.planner.data_collection import find_latest_model_path, load_model, save_model
from jacta.planner.planner.graph import Graph, sample_random_goal_states, sample_random_start_states
from jacta.planner.planner.parameter_container import ParameterContainer
from jacta.planner.planner.types import ActionMode
...
class Learner:
    ...
    def actor_actions(self, actor: Actor, node_ids: IntTensor, action_time_step: float) -> FloatTensor:
        sub_goals = self.replay_buffer.learning_goals[node_ids]
        with torch.no_grad():
            actor_actions = actor.select_action(
                self.state_norm, self.replay_buffer.states[node_ids], sub_goals, self.exploration_function
            )
        return actor_actions * self.params.action_range * action_time_step

    def relative_distances_to(
        self, data_container: Union[Graph, ReplayBuffer], ids: IntTensor, target_states: FloatTensor
    ) -> FloatTensor:
        node_distances = scaled_distances_to(self.plant, data_container.states[ids], target_states)
        root_ids = data_container.root_ids[ids]
        root_distance = scaled_distances_to(self.plant, data_container.states[root_ids], target_states)
        return node_distances / root_distance

    def reward_function(
        self, data_container: Union[Graph, ReplayBuffer], node_ids: FloatTensor, goals: FloatTensor
    ) -> Tuple[FloatTensor, FloatTensor]:
        relative_distances = self.relative_distances_to(data_container, node_ids, goals)
        if self.params.learner_use_sparse_reward:
            return -(relative_distances > self.current_success_distance).float(), relative_distances
        else:  # clipped relative distance as reward
            return -min(relative_distances, torch.ones_like(relative_distances)), relative_distances
    ...
    def planner_exploration(self, root_states: FloatTensor) -> FloatTensor:
        ...
        graph.add_nodes(root_ids, root_ids, root_states, root_start_actions, root_end_actions, root_relative_actions)
    ...
    def policy_rollout(self, temporary: bool = False) -> Tuple[FloatTensor, bool]:
        ...
        parent_id = replay_buffer.next_temporary_id if temporary else replay_buffer.next_learner_id
        ...
        for t in range(params.learner_trajectory_length - 1):
            ...
            if self.actor_expert.is_trained:
                ...
                if q >= q_expert:  # Select action from best actor (retains noise in exploration)
                    relative_actions = self.actor_actions(self.actor, node_ids, params.action_time_step)
                    used_expert = False
                else:
                    relative_actions = self.actor_actions(self.actor_expert, node_ids, params.action_time_step)
                    used_expert = True
            else:
                relative_actions = self.actor_actions(self.actor, node_ids, params.action_time_step)
                used_expert = False
            ...
            match self.params.action_start_mode:
                case ActionMode.RELATIVE_TO_CURRENT_STATE:
                    start_actions = replay_buffer.states[node_ids][:, self.plant.actuated_pos]
            ...
            match params.action_end_mode:
                case ActionMode.RELATIVE_TO_CURRENT_STATE:
                    actuated_pos = self.replay_buffer.states[node_ids][:, self.plant.actuated_pos]
                    sampled_end_actions = actuated_pos + relative_actions
                case ActionMode.RELATIVE_TO_PREVIOUS_END_ACTION:
                    sampled_end_actions = self.replay_buffer.end_actions[node_ids] + relative_actions
            ...
            new_states, _ = self.plant.dynamics(states, start_end_actions, params.action_time_step)
    ...
    def graph_rollout(self, temporary: bool = False) -> FloatTensor:
        ...
        padding_ids = torch.full((params.learner_trajectory_length - len(path_to_goal),), path_to_goal[0])
        path_ids = torch.concatenate((padding_ids, path_to_goal))

        root_ids = torch.full((params.learner_trajectory_length,), replay_buffer.next_learner_id)
        parent_ids = torch.arange(
            replay_buffer.next_learner_id - 1, replay_buffer.next_learner_id + params.learner_trajectory_length - 1
        )
    ...
    def set_demonstration_injection(self, final_success_rate: float) -> None:
        ...
        if params.learner_stop_converged_injection and self.final_success_distance > 0.5:
            self.inject_demonstration = False
        else:
            match self.params.learner_injection_strategy:
                case 0:  # just initially
                    self.inject_demonstration = self.injections < params.learner_max_initial_injections
    ...
    def train(self, num_epochs: int = 50) -> None:
        ...
        for epoch in range(num_epochs):
            ...
            for _ in range(params.learner_cycles):
                ...
                self.update_norm(replay_buffer.states[path_ids], replay_buffer.learning_goals[path_ids])
            ...
            final_success_rate, current_success_rate, relative_distance = self.eval_agent()
    ...
    def state_action_training_data(
        ...
        for _ in range(num_trajectories):
            search_index = torch.randint(0, params.num_parallel_searches, size=()).item()
            graph.change_sub_goal_states(sample_random_goal_states(plant, params))
            best_id = graph.get_best_id(reward_based=False, search_indices=torch.tensor([search_index])).item()
            root_id = graph.root_ids[best_id]
            path = graph.shortest_path_to(best_id, start_id=root_id)[-params.learner_trajectory_length :]
            if len(path) == 1:
                continue

            states = graph.states[path[0:-1]]  # current states
            relative_actions = graph.relative_actions[path[1:]]  # current actions are stored in the next state
            actor_actions = relative_actions / params.action_range / params.action_time_step
            goals = torch.tile(graph.sub_goal_states, (len(path[1:]), 1))  # achieved goals
            q_values = self.reward_function(graph, path[1:], goals)[0][:, None]  # initialize with achieved rewards
            trajectory_length = len(path)

            if q_values[-1] == 0:  # only use successful trajectories
                for i in range(len(q_values) - 2, -1, -1):
                    q_values[i] += discount_factor * q_values[i + 1]  # update with discounted future q-value
                ...
                actor_actions_data = torch.cat((actor_actions_data, actor_actions.clone()))
        ...
        return states_data, goals_data, actor_actions_data, q_values_data, trajectory_length_data
    ...
    def pretrain(
        ...
        states, goals, actor_actions, q_values, trajectory_lengths = self.state_action_training_data(
        ...
        for epoch in range(num_epochs):
            for _ in range(self.params.learner_batches):
                actor_loss = train_actor_imitation(actor, state_norm, states, goals, actor_actions)
                if train_critic:
                    critic_loss = train_critic_imitation(critic, state_norm, states, goals, actor_actions, q_values)
                else:
                    critic_loss = 0.0
            if self.verbose:
    ...
    def eval_agent(self) -> Tuple[float, float, float]:
        ...
        for i in range(learner_evals):
            ...
            sub_goals[i] = self.replay_buffer.learning_goals[self.replay_buffer.next_temporary_id - 1].clone()
        rewards, relative_distances = self.reward_function(self.replay_buffer, final_ids, sub_goals)
        final_success = torch.sum(1 - (relative_distances > self.final_success_distance).float())
        current_success = torch.sum(1 + rewards)  # reward is -1 or 0
        relative_distance = torch.mean(relative_distances)

        return final_success / learner_evals, current_success / learner_evals, relative_distance
