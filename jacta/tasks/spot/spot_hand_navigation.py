...
from jacta.plannermanipulation.mujoco_dynamics.utils import get_sensor_name
...
class SpotHandNavigation(SpotBase[SpotHandNavigationConfig]):
    ...
    def actuator_ctrlrange(self) -> np.ndarray:
        ...
        lower_bound = np.concatenate((-0.5 * np.ones(3), ARM_UNSTOWED_POS - 0.7 * np.array([1] * 6 + [0])))
        upper_bound = np.concatenate((0.5 * np.ones(3), ARM_UNSTOWED_POS + 0.7 * np.array([1] * 6 + [0])))
