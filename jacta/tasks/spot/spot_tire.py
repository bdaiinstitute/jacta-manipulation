...
class SpotTire(SpotBase[SpotTireConfig]):
    ...
    def reward(
        ...
        goal_reward = -config.w_goal * np.linalg.norm(tire_pos - config.goal_tire_pos, axis=-1).mean(-1)

        # Compute l2 distance from torso pos. to tire pos.
        torso_proximity_reward = -config.w_torso_proximity * np.linalg.norm(torso_pos - torso_goal, axis=-1).mean(-1)
        ...
        leg_proximity_reward = config.w_leg_proximity * (sensors[..., -4:] - 0.1).sum(-1).mean(-1)
        ...
        tire_linear_velocity_reward = -config.w_tire_linear_velocity * np.linalg.norm(states[..., -6:-3], axis=-1).mean(
            -1
        )
        tire_angular_velocity_reward = -config.w_tire_angular_velocity * np.linalg.norm(states[..., -3:], axis=-1).mean(
            -1
        )
    ...
    def reset(self) -> None:
        ...
        while np.linalg.norm(reset_pose[:3] - standing_pose) < robot_radius:
            ...
            reset_pose[2] = 0.275
            reset_pose[3:] = [1, 0, 0, 0]
        self.data.qpos = np.array([0, 0, 0.52, 1, 0, 0, 0, *STANDING_UNSTOWED_POS, *reset_pose])
    ...
    def actuator_ctrlrange(self) -> np.ndarray:
        ...
        lower_bound = np.concatenate((-0.5 * np.ones(3), ARM_UNSTOWED_POS - 0.7 * np.array([1, 1.2, 1, 1, 1, 1, 0])))
        upper_bound = np.concatenate((0.5 * np.ones(3), ARM_UNSTOWED_POS + 0.7 * np.array([1, 1.2, 1, 1, 1, 1, 0])))
