...
from jactamanipulation.mujoco_dynamics.utils import count_trace_sensors, get_mesh_data
...
class ViserMjModel:
    ...
    def __init__(self, target: ViserServer | ClientHandle, model: MjModel, show_ground_plane: bool = True):
        self._target = target
        self._model = model
    ...
    def add_geom(self, geom_name: str, geom: Any) -> None:
        ...
        match geom.type.item():
            ...
            case mujoco.mjtGeom.mjGEOM_SPHERE:
                self._geoms.append(
                    add_sphere(
                        self._target, geom.name, radius=geom.size[0], pos=geom.pos, quat=geom.quat, rgba=geom.rgba
                    )
                )
    ...
    def add_traces(self, num_traces: int = 0, all_traces_rollout_size: int = 0, trace_name: str = "trace") -> None:
    ...
    def set_traces(self, traces: np.ndarray | None, all_traces_rollout_size: int) -> None:
        ...
        if traces is None:
            ...
            if len(self._traces) <= 1 or self.all_traces_rollout_size != all_traces_rollout_size:
                self.remove_traces()
...
def set_spline_positions(
    handle: SplineCatmullRomHandle, positions: tuple[tuple[float, float, float], ...] | np.ndarray
) -> None:
