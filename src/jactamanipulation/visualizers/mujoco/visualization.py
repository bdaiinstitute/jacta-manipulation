...
from jactamanipulation.tasks.task import Task
from jactamanipulation.viser_app.io import ControlBufferKeys, IOContext, StateBufferKeys
...
class MjVisualization:
    ...
    def __init__(self, task: Task, server: ViserServer, context: IOContext, handles: dict):
    ...
    def update_visualization(self) -> None:
        ...
        if not self.context.task_updated_event.is_set():
            with self.context.state_lock:
                self.data.xpos[:] = self.context.state_buffer.get(StateBufferKeys.xpos, None)
                self.data.xquat[:] = self.context.state_buffer.get(StateBufferKeys.xquat, None)

            with self.context.control_lock:
                self.traces = self.context.control_buffer.get(ControlBufferKeys.traces, None)
