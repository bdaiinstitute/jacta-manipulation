...
from dexterity.spot_hardware.low_level.constants import ARM_STOWED_POS
...
class SpotBaseConfig(TaskConfig):
    ...
    default_command: np.ndarray = field(
        default_factory=lambda: np.array([0, 0, 0] + list(ARM_STOWED_POS) + [0] * 12 + [0, 0, 0.55])
    )
...
class SpotBase(MujocoTask[ConfigT]):
    ...
    def __init__(self, model_filepath: str, policy_filepath: str) -> None:
        ...
        self.system = create_systems_vector(self.model_filepath, self.policy_filepath, num_systems=1)

        self.physics_substeps: int = 2
        self.default_policy_command = np.array([0, 0, 0] + list(ARM_STOWED_POS) + [0] * 12 + [0, 0, 0.55])
    ...
    def rollout(
        ...
        full_commands = np.tile(self.default_policy_command, (num_threads, num_timesteps, 1))
        full_commands[..., self.command_mask] = controls

        policy_outputs = np.broadcast_to(
            additional_info["last_policy_output"], (num_threads, self.initial_policy_output.shape[-1])
        )
    ...
    def make_models(self, num_models: int) -> list[System]:
        ...
        return create_systems_vector(self.model_filepath, self.policy_filepath, num_systems=num_models)
