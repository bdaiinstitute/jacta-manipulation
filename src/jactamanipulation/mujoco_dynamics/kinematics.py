...
class IkSolver:
    ...
    orientation_cost: np.ndarray = field(default_factory=lambda: 1e-1 * np.array([1, 0, 0, 0, 0]))
    posture_cost: np.ndarray = field(default_factory=lambda: 1e-2 * np.array([1, 1, 20, 10, 10, 1] + [1] * 19))
    pos_threshold: float = 1e-2
    ori_threshold: float = 1.0
    max_iters: int = 20
    dt: float = 0.002
    damping: float = 1e-3
    frame_tasks_damping: np.ndarray = field(default_factory=lambda: np.array([1, 1, 1, 1, 1]))
    ...
    def _initialize_tasks(self) -> None:
        ...
        for frame_name, position_cost, orientation_cost, damping in zip(
            self.frame_names, self.position_cost, self.orientation_cost, self.frame_tasks_damping, strict=False
        ):
            setattr(
                self,
                f"{frame_name}_task",
        ...
        self._posture_task = mink.PostureTask(model=self.model, cost=self.posture_cost[: self.model.nv])
    ...
    def set_target_pose(self, target_poses: list[np.ndarray | None]) -> None:
        ...
        for frame_task, pose in zip(self.frame_tasks, target_poses, strict=False):
            if pose is not None:
                ...
                target_transform = mink.SE3.from_rotation_and_translation(rotation, translation)
    ...
    def solve(
        ...
        assert not all(pose is None for pose in target_poses), "At least need one target pose."
        ...
        if len(target_poses) != len(self.frame_tasks):
            ...
            target_poses = [target_poses[0]] + [None] * (len(self.frame_tasks) - len(target_poses))
        ...
        for i in range(self.max_iters):
            velocity = mink.solve_ik(
                self.configuration, tasks, self.dt, self.solver, limits=limits, damping=self.damping
            )
            ...
            for frame_task, pose in zip(self.frame_tasks, target_poses, strict=False):
                if pose is not None:
                    ...
                    pos_err = np.linalg.norm(error[:3]) * np.sign(frame_task.position_cost)
                    ori_err = np.linalg.norm(error[3:]) * np.sign(frame_task.orientation_cost)
            ...
            logging.warning(f"Reached maximum iterations ({self.max_iters}) without convergence.")
