...
from jacta.dynamics.simulator_plant import SimulatorPlant, scale_distances
...
def sample_related_sub_goal_states(
    params: ParameterContainer, goal_states: FloatTensor, start_states: FloatTensor, size: int = 1
) -> FloatTensor:
    """
...
def sample_feasible_states(
    plant: SimulatorPlant, bound_lower: FloatTensor, bound_upper: FloatTensor, size: int = 1
) -> FloatTensor:
...
def sample_random_start_states(plant: SimulatorPlant, params: ParameterContainer, size: int = 1) -> FloatTensor:
    return sample_feasible_states(plant, params.start_bound_lower, params.start_bound_upper, size=size)


def sample_random_goal_states(plant: SimulatorPlant, params: ParameterContainer, size: int = 1) -> FloatTensor:
    return sample_feasible_states(plant, params.goal_bound_lower, params.goal_bound_upper, size=size)


def sample_random_sub_goal_states(plant: SimulatorPlant, params: ParameterContainer, size: int = 1) -> FloatTensor:
...
class Graph:
    ...
    def calculate_proximity_rewards(self, ids: IntTensor) -> FloatTensor:
        ...
        if proximity_scaling.ndim == 0:
            rewards = -torch.norm(torch.matmul(proximity_scaling, self.sensors[ids].T).T, dim=-1)
        else:
    ...
    def _update_dynamics_info(self, ids: IntTensor, states: FloatTensor, end_actions: FloatTensor) -> None:
    ...
    def activate_all_nodes(self) -> None:
        ...
        self._update_dynamics_info(sub_node_ids, self.states[sub_node_ids], self.end_actions[sub_node_ids])
    ...
    def sorted_progress_ids(self, reward_based: bool, search_index: int = 0) -> IntTensor:
    ...
    def get_best_id(self, reward_based: bool = True, search_indices: Optional[IntTensor] = None) -> int:
