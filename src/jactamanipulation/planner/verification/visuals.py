...
class TrajectoryVisualizer:
    def __init__(
        ...
        base_path = Path(__file__).resolve().parent.parent.parent.parent
        self._directives_path = str(Path(base_path, "models/directives", self._params.vis_filename))
        ...
        for model in model_instances:
            for body_id in plant.GetBodyIndices(model):
                ...
                for geometry_id in inspector.GetGeometries(frame_id, Role.kIllustration):
                    ...
                    phong = properties.GetPropertyOrDefault("phong", "diffuse", Rgba(0, 0, 0, 0.0))
                    if color is None:
                    ...
                    scene_graph.AssignRole(plant.get_source_id(), geometry_id, properties, RoleAssign.kReplace)
    ...
    def setup_and_connect(self, trajectories: List[PiecewisePolynomial.FirstOrderHold]) -> None:
        self._builder = DiagramBuilder()
        self._builder.AddSystem(self._scene_graph)

        for i, traj in enumerate(trajectories):
            to_pose = self._builder.AddSystem(MultibodyPositionToGeometryPose(self.plants[i]))
            self._builder.Connect(
                to_pose.get_output_port(), self._scene_graph.get_source_pose_port(self.plants[i].get_source_id())
            )

            traj_system = self._builder.AddSystem(TrajectorySource(traj))
            self._builder.Connect(traj_system.get_output_port(), to_pose.get_input_port(0))
    ...
    def update_trajectories(self, trajectories: List[PiecewisePolynomial.FirstOrderHold]) -> None:
