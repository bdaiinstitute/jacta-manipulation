...
from jacta.experts.expert_sampler import ExpertSampler
from jacta.learning.networks import Actor
from jacta.learning.normalizer import Normalizer
from jacta.planner.data_collection import find_latest_model_path, load_model
...
class NetworkSampler(ExpertSampler):
    def __init__(
        ...
        if path:
            ...
            base_local_path = base_path + f"/examples/learner_examples/models/{task}/"
    ...
    def callback(self, node_ids: IntTensor) -> FloatTensor:
        ...
        with torch.no_grad():
            actor_actions = self.actor.select_action(self.state_norm, self.graph.states[node_ids], sub_goals)
