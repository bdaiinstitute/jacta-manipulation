...
from dexterity.jacta_planner.planner.linear_algebra import einsum_jk_ikl_ijl
from dexterity.jacta_planner.planner.types import ControlType
...
def create_action_trajectory(
    ...
    match type:
        case ControlType.ZERO_ORDER_HOLD:
            action_trajectory = torch.repeat_interleave(end_actions.unsqueeze(0), trajectory_steps, dim=0)
        case ControlType.FIRST_ORDER_HOLD:
            alpha = torch.linspace(0, 1, steps=trajectory_steps + 1)[1:].unsqueeze(1).to(start_actions.device)
...
def create_action_trajectories(
    ...
    match type:
        case ControlType.ZERO_ORDER_HOLD:
            action_trajectories = torch.repeat_interleave(end_actions.unsqueeze(1), trajectory_steps, dim=1)
        case ControlType.FIRST_ORDER_HOLD:
            alpha = torch.linspace(0, 1, steps=trajectory_steps + 1)[1:].unsqueeze(1).to(start_actions.device)
