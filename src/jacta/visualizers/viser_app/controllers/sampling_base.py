...
class SamplingBase(Controller):
    ...
    def __init__(
        ...
        self.states = np.zeros((self.config.num_rollouts, self.num_timesteps, self.model.nq + self.model.nv))
        self.sensors = np.zeros((self.config.num_rollouts, self.num_timesteps, self.model.nsensordata))
        self.rollout_controls = np.zeros((self.config.num_rollouts, self.num_timesteps, self.model.nu))
    ...
    def reset(self) -> None:
        ...
        self.candidate_controls = np.tile(self.controls, (self.config.num_rollouts, 1, 1))
