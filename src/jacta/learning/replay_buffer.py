...
class ReplayBuffer:
    ...
    def _initialize(self, plant: SimulatorPlant, params: ParameterContainer) -> None:
        ...
        self.max_nodes = (
            params.learner_evals + params.learner_max_trajectories
    ...
    def add_nodes(
        ...
        if temporary:
            ids = torch.arange(
                self.next_temporary_id, self.next_temporary_id + len(parent_ids)
            )
            self.next_temporary_id = ids[-1] + 1
        else:
            ids = torch.arange(
                self.next_learner_id, self.next_learner_id + len(parent_ids)
            )
            if ids[-1] >= self.max_nodes:  # end of buffer reached, start from beginning
                ids = torch.arange(
                    self.first_learner_id, self.first_learner_id + len(parent_ids)
                )
                parent_ids = ids - 1
    ...
    def sampling(
        ...
        her_episode_indices = torch.where(torch.rand(batch_size) < her_probability)[
            0
        ]  # relabling indices

        timestep_offset = (torch.rand(batch_size) * (T - timestep_indices)).to(
            torch.int
        )

        her_timestep_indices = (timestep_indices + 1 + timestep_offset)[
            her_episode_indices
        ]
        ...
        her_node_ids = learning_ids[
            episode_indices[her_episode_indices], her_timestep_indices
        ]

        states = self.states[current_node_ids].clone()
        # actions from current node are stored in next node
        actions = (
            self.relative_actions[next_node_ids].clone() / action_normalization_scaling
        )
