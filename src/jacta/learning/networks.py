...
def soft_update(network: nn.Module, target: nn.Module, tau: float) -> nn.Module:
    ...
    """
    for network_p, target_p in zip(
        network.parameters(), target.parameters(), strict=True
    ):
        ...
        if self._train:
            if torch.rand(1).item() >= self.eps:  # With noise for training
                actions += torch.normal(
                    mean=torch.zeros_like(actions), std=torch.ones_like(actions) * 0.2
                )
                torch.clip(
                    actions, -self.action_clip, self.action_clip, out=actions
                )  # In-place op
                ...
                actions = (
                    torch.rand(actions.shape) * 2 * self.action_clip - self.action_clip
                )
...
class DDP(nn.Module):
    ...
    def __init__(
        self, size_s: int, size_a: int, nlayers: int, layer_width: int
    ) -> None:
...
class Critic:
    ...
    def __init__(
        ...
        """
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.critic_net = CriticNetwork(size_s, size_a, nlayers, layer_width).to(
            self.device
        )
        self.target_net = CriticNetwork(size_s, size_a, nlayers, layer_width).to(
            self.device
        )
...
def train_actor_critic(
    ...
    with torch.no_grad():
        ...
        min_value = (
            -1
            * (1 - discount_factor**replay_buffer.params.learner_trajectory_length)
            / (1 - discount_factor)
        )
    ...
    if actor_expert.is_trained:
        with torch.no_grad():
            ...
            value = torch.max(
                value_expert, value
            )  # use highest possible value as reference for Q
    else:
        expert_share = 0.0
